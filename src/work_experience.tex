...
\newcommand{\resumeThirdListStart}{\begin{itemize}[leftmargi=2.3em, itemsep=1pt, label=$\circ$]}
...
데이터 무결성, 안정성, 그리고 "누구나 접근 가능한 신뢰 가능한 데이터”를 제공하는 것에 자신있어요.

%-----------PROJECTS-----------------

\ProjectTitle{개발자 경험 주도 데이터플랫폼 모노레포 도입}{2025.12 - 2026.02}

\textbf{Description}: 데이터플랫폼 내 여러 파이프라인과 라이브러리 코드를 단일 저장소(Monorepo)에서 효율적으로 관리하기 위한 환경을 구축했다.  
          Python의 차세대 패키지 관리 도구인 `uv`를 활용하여 워크스페이스를 구성하고, 공통 라이브러리를 `symlink`로 연결하여 중복을 제거했으며  
          모든 CI/CD 파이프라인은 GitLab을 통해 통합 관리하여 개발 생산성과 코드 일관성을 높였다.

\resumeSubProjectListStart
  % --- Problem ---
  \resumeSubProject{\textbf{Problem}}{%
    \resumeSubBulletsStart
      \item 여러 개의 분산된 레포지토리로 인한 파편화된 의존성 관리의 어려움
      \item 공통 라이브러리 변경 시, 이를 사용하는 모든 프로젝트에 일일이 버전을 업데이트하고 배포해야 하는 번거로움
      \item 프로젝트 간 코드 일관성 유지가 어렵고, 신규 프로젝트 셋업 비용 증가
      \item 전체 플랫폼의 변경 사항을 한눈에 파악하기 어렵고, 통합 테스트의 부재
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Action}}{
    \resumeSubBulletsStart
      \item Python 가상 환경 및 패키지 관리를 위해 `uv` 도입, 빠른 속도와 `pip` 호환성으로 개발 환경 통일
      \item Monorepo 내에 `libs`, `pipelines`, `services` 등 디렉토리 구조 표준화
      \item 공유 라이브러리는 `uv`의 워크스페이스 기능을 활용하고 `symlink`로 각 프로젝트에 연결하여 중복 없이 참조하도록 구성
      \item GitLab CI/CD 파이프라인 설계: `changes` 키워드를 활용하여 변경된 디렉토리(프로젝트)만 선택적으로 빌드/테스트/배포
      \item 레포지토리 전체의 코드 품질을 관리하기 위해 `pre-commit` 훅과 통합 린트(lint) 및 포매팅(formatting) 적용
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Result}}{%
    \resumeSubBulletsStart
      \item 공통 라이브러리 업데이트 및 전파 시간 80\% 단축
      \item `uv` 도입으로 로컬 개발 환경 구성 및 CI/CD 파이프라인의 패키지 설치 속도 50\% 이상 개선
      \item 신규 파이프라인 프로젝트 생성 시 보일러플레이트 자동화로 초기 설정 시간 단축
...
%-------------------------------------------
\ProjectTitle{Data Quality Gateㅡwith Runtime/Semantic}{2025.11 - 2026.01}
