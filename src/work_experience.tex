...
\ProjectTitle{데이터 플랫폼 컨테이너 운영 환경 전환}{2026.04 - }

\textbf{Description}: 데이터 플랫폼 운영 환경을 Kubernetes/EKS 기반 컨테이너 환경으로 옮기는 작업을 주도했다.
...
\resumeSubProjectListStart
  ...
  \resumeSubProject{\textbf{Problem}}{%
    \resumeSubBulletsStart
      \item 데이터 플랫폼 컴포넌트 운영이 컨테이너 기반으로 바뀌면서 배포 자산을 코드로 관리할 필요가 생김
      \item Managed 서비스 이용 및 비 컨테이너 환경으로 구성된 플랫폼으로 빗경별 차이를 추적하기 어렵고 운영 실수 가능성이 높았음
      \item 플랫폼 운영 컴포넌트가 늘어날수록 배포 방식, Secret 관리, 환경별 설정을 같은 기준으로 유지하기 어려움
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Action}}{%
    \resumeSubBulletsStart
      \item EKS 클러스터 신규 구성
      \item Kubernetes/EKS 운영을 전제로 Helm Chart 와 ArgoCD Application/ApplicationSet 구조 구성
      \item ArgoCD, AWS Load Balancer Controller, Secrets Store CSI Driver 등 플랫폼 운영 컴포넌트의 Chart 및 환경별 values 정리
      \item Observability 를 위한 Datadog 활용(datadog agent operator)
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Result}}{%
    \resumeSubBulletsStart
      \item 데이터 플랫폼 컴포넌트를 Kubernetes/Helm 기반으로 운영할 수 있는 배포 자산 구조를 마련
      \item 환경별 values 와 Secret 사용 여부를 함께 점검할 수 있어 배포 전 검토 부담을 줄임
      \item 신규 플랫폼 컴포넌트 추가 시 Helm values 와 ArgoCD 매니페스트를 기준으로 배포 흐름을 확장할 수 있게 됨
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Impact}}{%
    \resumeSubBulletsStart
      \item 데이터 플랫폼 운영 방식을 서버 중심에서 컨테이너 중심으로 옮기는 전환점 마련
      \item Airflow 중심의 파이프라인 운영 경험을 넘어 Kubernetes, Helm, ArgoCD, Secret 관리까지 포함한 플랫폼 운영 역량 확보
      \item 이후 Flink, Kafka Connect, 플랫폼 운영 컴포넌트에도 같은 컨테이너 운영 패턴을 적용할 수 있는 토대 확보
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Self-Reflection}}{%
    \resumeSubBulletsStart
      \item 컨테이너 전환은 애플리케이션을 이미지로 만드는 일에 그치지 않고, 배포 방식과 운영 설정 관리까지 함께 바꾸는 작업임을 체감
      \item Helm Chart, ArgoCD, Secret 관리가 맞물려야 안정적인 플랫폼 운영이 가능하다는 점을 배움
      \item 데이터 엔지니어링 업무에서도 Kubernetes 기반 운영 이해가 중요해지고 있음을 실무적으로 경험
    \resumeSubBulletsEnd
  }
\resumeSubProjectListEnd

%-------------------------------------------
\ProjectTitle{Managed Flink 기반 CDC Iceberg Sink 개발}{2026.04 - 2026.05}

\textbf{Description}: Airflow orchestration 코드와 분리해 Managed Flink 애플리케이션을 관리할 수 있는 구조를 만들었다.
Kafka CDC 이벤트를 Flink Table API로 읽어 Iceberg 테이블에 적재하는 Sink 애플리케이션을 개발했고,
changelog/materialized 적재 모드와 Glue Catalog, S3 warehouse naming rule 을 함께 정리했다.

\resumeSubProjectListStart
  % --- Problem ---
  \resumeSubProject{\textbf{Problem}}{%
    \resumeSubBulletsStart
      \item CDC 이벤트를 장기 분석이 가능한 Iceberg 테이블로 적재할 공통 애플리케이션 구조가 필요했음
      \item Kafka topic, Glue database, S3 warehouse 경로가 환경/DB 타입별로 달라 명명 규칙과 설정 주입 방식을 정리해야 했음
      \item Flink 애플리케이션을 Airflow DAG 코드와 분리하여 빌드/테스트/배포 가능한 단위로 관리할 필요가 있었음
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Action}}{%
    \resumeSubBulletsStart
      \item Gradle multi-module 기반 Managed Flink 애플리케이션 구조 구성 및 Shadow JAR 빌드 체계 마련
      \item Flink Table API 기반 Sink 구현: Kafka CDC 이벤트를 읽어 Iceberg 테이블로 적재
      \item changelog/materialized 적재 모드 지원으로 원본 이벤트 누적과 최신 상태 테이블 구성을 분리
      \item AWS Managed Flink runtime properties, CLI args, 기본 properties 파일을 조합해 설정을 주입하도록 설계
      \item Glue database, catalog alias, S3 warehouse path naming rule 을 환경/DB 타입 기준으로 정리
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Result}}{%
    \resumeSubBulletsStart
      \item Airflow orchestration 과 Flink stream processing 책임을 분리하여 애플리케이션 관리 경계를 명확히 함
      \item CDC topic 을 Iceberg 기반 분석 테이블로 적재하는 재사용 가능한 Sink 애플리케이션 확보
      \item 환경별 설정과 테이블 naming rule 을 코드/설정으로 관리하여 운영 실수 가능성을 줄임
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Impact}}{%
    \resumeSubBulletsStart
      \item Kafka CDC, Flink, Iceberg 로 이어지는 Lakehouse 적재 패턴을 구현할 기술 기반을 직접 구축
      \item 배치 중심 Airflow 운영 경험에서 스트리밍 처리와 테이블 포맷 운영까지 역할 확장
      \item Flink, Iceberg, Glue Catalog, S3 warehouse 를 연결하며 실시간 데이터 플랫폼 아키텍처 이해도 향상
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Self-Reflection}}{%
    \resumeSubBulletsStart
      \item Flink 애플리케이션은 DAG 스크립트와 달리 빌드 산출물, 런타임 설정, 테이블 스키마를 함께 고려해야 함을 체감
      \item CDC 이벤트를 Iceberg 테이블로 적재하면서 append-only 로그와 materialized view 성격의 테이블 설계 차이를 더 명확히 이해
      \item 데이터 파이프라인 운영 범위가 orchestration 을 넘어 stream processing, table format, catalog 관리로 확장됨을 경험
    \resumeSubBulletsEnd
  }
\resumeSubProjectListEnd
...
\textbf{Description:}
  데이터플랫폼 내 여러 파이프라인과 라이브러리 코드를 단일 저장소(Monorepo)에서 효율적으로 관리하기 위한 환경을 구축했다.  
  Python의 차세대 패키지 관리 도구인 `uv`를 활용하여 워크스페이스를 구성하고, 공통 라이브러리를 `symlink`로 연결하여 패키지 의존성과 중복을 제거했다.
  그리고 어플리케이션 CI/CD 는 GitLab Runner 를 이용해 통합 관리, 개발 생산성과 코드 일관성을 높였다.

\resumeSubProjectListStart
  % --- Problem ---
  \resumeSubProject{\textbf{Problem}}{%
    \resumeSubBulletsStart
      \item 여러 개의 분산된 레포지토리로 인한 파편화된 의존성 관리 어려움
      \item 프로젝트 간 코드 일관성 유지가 어렵고 신규 Repository 셋업 비용 증가
      \item CI/CD 라이프사이클 변경 시 높은 커뮤니케이션 비용(개발 생산성 저하)
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Action}}{%
    \resumeSubBulletsStart
      \item 패키지 관리도구 `uv` workspace 와 hatch buildbackend 활용. 빠른 속도와 분리/구조화된 distribution 관리
      \item `symlink`로 각 프로젝트에 연결하여 중복 없이 참조하도록 구성
      \item CICD 스크립트 모듈개발: `changes` \& 변경된 디렉토리(프로젝트)만 선택적으로 빌드/테스트/배포 환경 구성
      \item 코드 품질 관리를 위한 `pre-commit(w/ prek)` hook 도입
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Result}}{%
    \resumeSubBulletsStart
      \item 공통 라이브러리 업데이트 및 전파 시간 단축(80\%)
      \item 로컬 개발 환경 구성 및 CI/CD 파이프라인 사이클 속도 50\% 개선
      \item 신규 파이프라인 생성 시 통합테스트로 변경전파 확인 시간 감소
      \item Monorepo 구조 하에서 코드 변경의 영향 범위를 명확히 파악 가능해져 배포 안정성 증대
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Impact}}{%
    \resumeSubBulletsStart
      \item 플랫폼 전체의 기술적 통일성을 확보하고, "하나의 팀"처럼 협업하는 문화의 기반을 마련
      \item 코드 재사용성이 극대화되고, 개발자들이 핵심 비즈니스 로직에 더 집중할 수 있는 환경 제공
      \item 전사적으로 파편화된 Python 프로젝트들의 관리 표준을 제시하는 선례를 남김
      \item 신규 입사자가 플랫폼의 전체 코드 베이스를 빠르게 파악하고 적응할 수 있도록 기여
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Self-Reflection}}{
    \resumeSubBulletsStart
      \item Monorepo는 단순히 코드를 합치는 것이 아니라, 도구(uv, CI/CD), 규칙, 문화가 함께 가야 성공할 수 있음을 체감
      \item `uv`와 같은 최신 도구를 검토하고 도입하며 기술 트렌드를 실무에 적용하는 경험을 쌓음
      \item GitLab CI/CD의 고급 기능(e.g., dynamic child pipelines)을 활용하여 복잡한 Monorepo의 빌드 시스템을 효율적으로 설계하는 역량을 키움
      \item 대규모 코드 베이스를 관리하면서 발생하는 장단점을 깊이 이해하고, 트레이드오프를 고려한 아키텍처 설계의 중요성을 배움
      \item 모노레포 영감은 Airflow의 파이썬 패키지 의존성 관리 어려움에서 비롯되었으며, '패키지 지옥'에서 벗어나고자 하는 의지가 컸음
      \item 플랫폼이라는 하나의 놀이터를 제공하는 관점에서, 레포지토리가 분리되어 있는 구조는 개발자의 사용자 경험을 저해한다고 판단했음
    \resumeSubBulletsEnd
  }
\resumeSubProjectListEnd

%-------------------------------------------
\ProjectTitle{Data Quality Gate 개발ㅡwith Runtime/Semantic}{2025.06 - 2025.08}

\textbf{Description}: (IN BITHUMB) 데이터 파이프라인 전반에 걸쳐 데이터 품질을 보장하기 위한 Data Quality Gate를 구축했다.  
          Ingestion 시점의 'Runtime' 검증과 Transformation 시점의 'Semantic' 검증으로 나누어 데이터 신뢰도를 높이는 것을 목표로 했으며,  
          Great Expectations 0.8.x 버전을 활용하여 설정 파일 기반의 자동화된 데이터 품질 파이프라인을 구현했다.

\resumeSubProjectListStart
  % --- Problem ---
  \resumeSubProject{\textbf{Problem}}{%
    \resumeSubBulletsStart
      \item 데이터 파이프라인에 유입되는 Null, 잘못된 형식의 데이터로 인한 후반 프로세스 장애 발생
      \item 데이터 품질 이슈 발생 시 원인 파악 및 수정에 많은 시간 소요
      \item BI 대시보드 및 분석 결과의 신뢰도 하락으로 데이터 기반 의사결정의 걸림돌
      \item 수동으로 데이터 품질을 검증하는 데 드는 비효율적인 리소스
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Action}}{
    \resumeSubBulletsStart
      \item Great Expectations(0.8.x) 도입하여 데이터 품질 검증 자동화 환경 구축
      \item 'Runtime' 검증: Ingestion 단계에서 데이터 형식, Null 여부 등 기본적인 정합성 체크
      \item 'Semantic' 검증: Transformation 단계에서 비즈니스 로직 기반의 통계적 분포, 관계 무결성 등 의미적 품질 체크
      \item 설정 파일(YAML) 기반으로 검증 규칙(Expectation)을 정의하고, CLI를 통해 파이프라인에 통합
      \item 품질 검증 실패 시 Airflow Task 실패 처리 및 슬랙 알림 연동으로 즉각적인 대응 체계 마련
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Result}}{%
    \resumeSubBulletsStart
      \item 데이터 품질 이슈로 인한 파이프라인 실패율 30\% 감소
      \item 품질 검증 리포트(Data Docs) 자동 생성으로 데이터 프로파일링 및 현황 파악 용이성 증대
      \item 데이터 오류 발생 시 원인 추적 시간 단축 (MTTR 개선)
      \item 신규 데이터 파이프라인 구축 시 품질 검증 절차 표준화 및 적용 시간 단축
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Impact}}{%
    \resumeSubBulletsStart
      \item "신뢰할 수 있는 데이터" 제공이라는 데이터팀의 핵심 가치 실현
      \item 데이터 분석가 및 현업 사용자의 데이터 신뢰도 향상 및 데이터 활용도 증진
      \item 품질 게이트 도입으로 데이터 거버넌스 체계의 기술적 기반 마련
      \item 자동화된 품질 검증 문화를 팀에 정착시키는 계기 마련
    \resumeSubBulletsEnd
  }
  ...
  \resumeSubProject{\textbf{Self-Reflection}}{
    \resumeSubBulletsStart
      \item 데이터 품질은 파이프라인의 특정 지점이 아닌, End-to-End 관점에서 관리되어야 함을 체감
      \item Great Expectations 같은 도구를 활용하여 "기대를 코드로 관리"하는(Expectations as Code) 경험 확보
      \item 'Runtime'과 'Semantic'으로 품질 검증의 책임을 분리함으로써 복잡한 파이프라인을 더 체계적으로 관리할 수 있음을 배움
      \item 단순히 도구를 도입하는 것을 넘어, 조직의 데이터 문화에 맞는 검증 프로세스를 정립하는 것이 중요함을 깨달음
    \resumeSubBulletsEnd
  }
\resumeSubProjectListEnd

\ProjectTitle{데이터 플랫폼 아키텍처의 재설계ㅡSoftware Architecture Document 를 중심으로}{2025.09 - 2025.11}
...
\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{sad-diagram.png}
\caption{SAD CI/CD Pipeline Overview}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{data-api-diagram.png}
\caption{Data API Diagram}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{cdc-diagram.png}
\caption{CDC Pipeline Overview}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{migration-kafka-connect-diagram.png}
\caption{Kafka Connect Overview}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=1\textwidth]{dbt-cicd-diagram.png}
