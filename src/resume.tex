  ...
  \resumeSubHeadingListStart
    \resumeSubheading
      ...
      \resumeItemListStart
        \resumeItem{High-volume, Optimized Data Platform}
          {
            ...
            특히 사용자 혹은 서비스에 어떤 인터페이스로 데이터를 제공할 수 있을지 고민하고, 큰 데이터 생명주기(LifeCycle) 를 경험했어요.
          }
        \resumeItem{Interface for DataServing}
          {
            서비스 DB의 조회 부담을 줄이고자 집계가 필요한 데이터를 DW에 위임하고, 이를 제공하는 API를 개발했어요.
            Reverse ETL을 도입하여 데이터 서빙의 역할과 책임을 명확히 분리하고, 모노레포 기반으로 서비스 간 의존성을 제거하여 생산성을 높였어요.
          }
        \resumeItem{Structured Architecture Design}
          {
        ...
        \resumeItem{Data-driven Team Contribution}
          {
            '주도하는' 데이터팀을 목표로, 데이터의 비즈니스 기여도를 측정하고 성과를 추적하는 플랫폼을 만들고 운영했어요. 이를 통해 데이터 기반의 의사결정 문화를 전파했어요.
          }
      \resumeItemListEnd
