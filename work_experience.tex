...
\newcommand{\ProjectTitle}[2]{
  ...
  \addcontentsline{toc}{section}{#1} % 목차를 쓰면 유효, 없으면 무시됨
}
...
%-----------PROJECTS-----------------
\ProjectTitle{PROJECT: Software Architecture Document 기반 데이터 플랫폼 재설계}{2025.09 - 2025.11}

\textbf{Description}: (IN BITHUMB) 데이터 플랫폼 전반의 복잡도가 증가하면서 시스템 구조·운영 방식·의사결정 이유를 명확히 설명하기 어려운 상황이었다.  
      이를 해결하기 위해 CMU SEI의 Software Architecture Document(SAD) 템플릿을 기반으로 데이터 플랫폼 전체의 아키텍처를 정리하고, Architecture Decision Record(ADR)를 도입하여  
      기술 선택과 운영 기준을 문서화했다. 모든 문서는 마크다운 형태로 작성되었으며 mkdocs \& Gitlab CI/CD 와 S3 static web hosting 으로 제공했다.

\resumeSubProjectListStart
  % --- Problem ---
  \resumeSubProject{\textbf{Problem}}{%
    \resumeSubBulletsStart
      \item 방대한 구성요소에 비해 산발적으로 흩어져 있는 문서로 신규 온보딩 지연(최소 14일, 최대 한 달)
      \item "왜 Redshift 인가?|왜 운영정책은 이러지?" 의 의사결정이 쌓이지 않아 반복적인 논의 발생
      \item 운영 관리문서 부재, 조직 간 용어정의 불일치
    \resumeSubBulletsEnd
  }
...
\resumeSubProject{\textbf{Action}}{
  \resumeSubBulletsStart
    \item SEI SAD 템플릿 기반 아키텍처 재설계(CnC/Deployment 전통 + 데이터 플랫폼 특화 파이프라인 뷰 등)
    \item With ADR(Architecture Decision Record): 변경 이력 관리 및 리뷰 문화 활성화를 도입, "왜?" 의 WAREHOUSE 구성
    \item 문서 빌드 파이프라인 개발: mkdocs 로 정적 문서 빌드(Merge 혹은 MR 이벤트CI/CD), 코드 수정에 따른 문서 미변경 시 알람 및 수정사항 반영 제안
    \item 운영 런북(온보딩가이드): 플랫폼 운영(트러블슈팅) 문서 작성 및 문화 공유
  \resumeSubBulletsEnd
}
...
\resumeSubProject{\textbf{Result}}{%
  \resumeSubBulletsStart
    \item 신규 엔지니어 온보딩 기간 ??일 에서 약 ??일 단축
    \item 문서 WAREHOURSE 로 MTTR 개선(?? 에서 ??)
    \item ADR 로 인한 불필요한 커뮤니케이션(반복논의) ?? 감소
    \item 구성요소/파이프라인/운영이 어루어져 "이렇게 동작해" 설명 가능
  \resumeSubBulletsEnd
}
...
\resumeSubProject{\textbf{Impact}}{%
  \resumeSubBulletsStart
    \item 코드에 매몰되어 있던 나를 설계 철학/운영/품질속성 기반 아키텍처로 바라보는 계기
    \item 팀 간 책임 경계를 명확히 하여 운영 혼선/중복(DO NOT REPEAT YOURSELF) 구현/아쉬운 기술 선택 감소
...
\resumeSubProjectListStart
  ...
  \resumeSubProject{\textbf{Result}}{%
    \resumeSubBulletsStart
      \item DW 집계 SLA 99.9\%
      \item 타 서비스 API 대비 40\% 리소스로 최대 500QPS 
    \resumeSubBulletsEnd
  }
...
\resumeSubProjectListStart
  ...
  \resumeSubProject{\textbf{Result}}{
    \resumeSubBulletsStart
      ...
      \item 평균 CDC 지연 ??초 → ??초로 감소  
      \item Snapshot/Lock 관련 DB 성능 이슈 0건  
      \item Connect 장애 MTTR ??분 → ??분로 단축  
      \item CDC 이벤트 기반 모델을 활용한 서비스·분석 팀의 신규 활용 케이스 ?? 건 증가  
      \item 운영 지표 시각화로 장애 감지 속도 ?? 분 → ?? 단축
    \resumeSubBulletsEnd
  }
...
\resumeSubProjectListStart
  ...
  \resumeSubProject{\textbf{Result}}{%
    \resumeSubBulletsStart
      \item 한 달 운영 기준 99.9\% 이상의 BigQuery 적재 성공률 달성 (인프라 이슈 제외)
      \item 장애 감지 시간 {}분 → {}초로 단축, MTTR {}분
  ...
  \resumeSubProject{\textbf{Impact}}{%
    \resumeSubBulletsStart
      \item Kafka Connect를 통해 실시간 수집 파이프라인을 표준화하고 다양한 소스/타깃 시스템으로 확장 가능한 데이터 플랫폼 인터페이스
...
\resumeSubProjectListStart
  ...
  \resumeSubProject{\textbf{Result}}{
    \resumeSubBulletsStart
      \item 기존 Bamboo 대비 빌드 속도 {}\% 개선, 실패 원인 식별 시간 {}분 → {}분으로 단축
      \item Slim CI 도입으로 변경 모델 중심 빌드가 정착, 전체 빌드 비용 {}\% 절감
...
\resumeSubProjectListStart
  ...
  \resumeSubProject{\textbf{Result}}{%
    \resumeSubBulletsStart
      ...
      \item MageAI 기반 보고 자동화로 반복 작업 시간 {}분 → {}분으로 단축
      \item 내부 크루가 스스로 데이터 위치를 파악하고 활용하는 self-serve 문화 정착
    \resumeSubBulletsEnd
  }
